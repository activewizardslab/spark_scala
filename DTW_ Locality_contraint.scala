...
var w = 10 // define the window size 
val channels = List("Fc5", "Fc3", "Fc6")
val path1 = "/home/terrapin/Downloads/eegmmidb_S002_S002R01_5000.csv"
val path2 = "/home/terrapin/Downloads/eegmmidb_S002_S002R01_5000.csv"
...
// calculate DTW
def DTW_naive(t1: org.apache.spark.rdd.RDD[(Float, Long)], t2: org.apache.spark.rdd.RDD[(Float, Long)], w: Int) : Double = {	
		...
		var ww = w
		ww = math.max(ww, math.abs(t1_n-t2_n)) // find the maximum between window size  and time points difference
		val zero = 0.0
		...
		val j=0;
		var minimum = 0.0
		for (i <- 0 until t1_n; j <- math.max(0, i-ww) until math.min(t2_n, i+ww)){ // iterate through matrix	
...
def result_writting(path1: String, path2: String, channels: List[String], w: Int): scala.collection.mutable.MutableList[(String, Double)] = {
...
result_writting(path1, path2, channels, w)
